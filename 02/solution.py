def parsePolicy(line):
    parts = line.split(" ")
    bounds = [int(x) for x in parts[0].split("-")]
    letter = parts[1][0]
    string = parts[2]
    # this is the data structure for a policy
    return (bounds, letter, string)


def isPolicyValidOld(policy):
    bounds = policy[0]
    letter = policy[1]
    string = policy[2]
    count = 0
    for i in string:
        if i == letter:
            count += 1
    return count >= bounds[0] and count <= bounds[1]


def isPolicyValidNew(policy):
    bounds = policy[0]
    letter = policy[1]
    string = policy[2]
    rulesMet = 0
    rulesMet += 1 if string[bounds[0] - 1] == letter else 0
    rulesMet += 1 if string[bounds[1] - 1] == letter else 0
    return rulesMet == 1


def solve(puzzle_input):
    policies = [parsePolicy(p) for p in puzzle_input.strip().split("\n")]

    # part 1
    totalValidPolicies = 0
    for policy in policies:
        if isPolicyValidOld(policy):
            totalValidPolicies += 1
    print(totalValidPolicies)

    # part 2
    totalValidPolicies = 0
    for policy in policies:
        if isPolicyValidNew(policy):
            totalValidPolicies += 1
    print(totalValidPolicies)

def parse_rules(rules_string):
    rule_strings = [x.strip().split(": ") for x in rules_string.split("\n")]
    rules_map = {}
    for rule_string in rule_strings:
        id = int(rule_string[0])
        parts = rule_string[1].split(" | ")
        rule_matches = []
        for part in parts:
            if '"' in part:
                rule_matches.append(part.replace('"', ''))
            else:
                rule_matches.append([int(x) for x in part.split(" ")])
        rules_map[id] = rule_matches
    return rules_map


def match(message, rules, rule_id=0):
    '''
    ...
    for rule_set in rules[rule_id]:
        '''
        for each rule set, see if any of them match.
        '''

        '''tuples of matched-string and remaining string to match'''
        matches_so_far = [('', message)]

        for rule in rule_set:
            '''
            see if this rule matches the beginning of the message.
            unfortunately, now that rules can loop, we need to account for muliple matches for a given rule
            and message
            '''
            new_matches_so_far = []
            for match_so_far in matches_so_far:
                matched_message_so_far = match_so_far[0]
                remaining_message_to_match = match_so_far[1]

                if (rule == "a" or rule == "b"):
                    matched_messages = [rule] if rule == message[0] else None
                else:
                    matched_messages = match(
                        remaining_message_to_match, rules, rule)

                if matched_messages:
                    for matched_message in matched_messages:
                        if matched_message and remaining_message_to_match.startswith(matched_message):
                            new_matches_so_far.append(
                                (matched_message_so_far + matched_message, remaining_message_to_match[len(matched_message):]))

            matches_so_far = new_matches_so_far
            if len(matches_so_far) == 0:
                '''if we have found no matches so far, no other rules in this set matter'''
                break

        matches.extend([x[0] for x in matches_so_far])

    return matches


def is_valid(message, rules):
    matched = match(message, rules)
    return True if len(matched) > 0 and matched[0] == message else False


def solve(puzzle_input):
    rules = parse_rules(puzzle_input.strip().split("\n\n")[0])
    messages = puzzle_input.strip().split("\n\n")[1].split("\n")

    total_valid = 0
    for message in messages:
        total_valid += 1 if is_valid(message, rules) else 0
    print(total_valid)
    ...
    11: 42 31 | 42 11 31
    """.strip())
    rules[8] = replacement_rules[8]
    rules[11] = replacement_rules[11]

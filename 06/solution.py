...
def solve(puzzle_input):
    def get_unique_answers(answers, everyone=False):
        if everyone:
            answers_per_person = [set(x) for x in answers.strip().split('\n')]
            result = reduce((lambda x, y: x.intersection(y)),
                            answers_per_person)
            return result
        unique = set(answers)
        unique.discard('\n')

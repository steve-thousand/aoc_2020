...
def validate_height(height_string):
    if re.match("^\\d+cm$", height_string):
        height = int(height_string[0:-2])
        return height >= 150 and height <= 193
    if re.match("^\\d+in$", height_string):
        height = int(height_string[0:-2])
        return height >= 59 and height <= 76
    return False


checksum_field_parts = {
    "byr": (0, lambda x: len(x) == 4 and int(x) >= 1920 and int(x) <= 2002),
    "iyr": (1, lambda x: len(x) == 4 and int(x) >= 2010 and int(x) <= 2020),
    "eyr": (2, lambda x: len(x) == 4 and int(x) >= 2020 and int(x) <= 2030),
    "hgt": (3, validate_height),
    "hcl": (4, lambda x: bool(re.match("^#[0-9a-f]{6}$", x))),
    "ecl": (5, lambda x: bool(re.match("^(amb|blu|brn|gry|grn|hzl|oth)$", x))),
    "pid": (6, lambda x: bool(re.match("^\\d{9}$", x))),
    "cid": (7, lambda x: True)
}


def is_passport_valid(passport, validate_fields=False):
    checksum = 0
    for field in passport:
        if validate_fields:
            is_valid = checksum_field_parts[field[0]][1](field[1])
            if not is_valid:
                return False
        checksum += 1 << checksum_field_parts[field[0]][0]
    return checksum == 255 or checksum == 127


def solve(puzzle_input):

    def passport_field_parse(passport_field_string):
        parts = passport_field_string.split(":")
        return (parts[0], parts[1])

    passports = [[passport_field_parse(y) for y in x.split()]
    ...
    valid_count = 0
    for passport in passports:
        valid = is_passport_valid(passport, True)
        valid_count += 1 if valid else 0
    print(valid_count)

class Recipe:
    ...
    def get_intersection_ingredients(self, other):
        return self.ingredients.intersection(other.ingredients)

    def get_intersection_allergens(self, other):
        return self.allergens.intersection(other.allergens)

    @staticmethod
    def parse(string):
        parts = string.split(" (contains ")
        ingredients = parts[0].split(" ")
        allergens = parts[1][:-1].split(", ")
        return Recipe(set(ingredients), set(allergens))
...
class RecipeReducer:
    def __init__(self, recipes):
        ...
        self.ingredients_by_recipe = {}
        self.recipes_by_ingredient = {}
        self.allergens_by_recipe = {}
        self.recipes_by_allergen = {}
        self.identified_allergens = {}
        self.identified_ingredients = {}
        for i, recipe in enumerate(recipes):
            for ingredient in recipe.ingredients:
                self.unique_ingredients.add(ingredient)
                if i not in self.ingredients_by_recipe:
                    self.ingredients_by_recipe[i] = set()
                self.ingredients_by_recipe[i].add(ingredient)
                if ingredient not in self.recipes_by_ingredient:
                    self.recipes_by_ingredient[ingredient] = set()
                self.recipes_by_ingredient[ingredient].add(i)
            for allergen in recipe.allergens:
                if i not in self.allergens_by_recipe:
                    self.allergens_by_recipe[i] = set()
                self.allergens_by_recipe[i].add(allergen)
                if allergen not in self.recipes_by_allergen:
                    self.recipes_by_allergen[allergen] = set()
                self.recipes_by_allergen[allergen].add(i)
        return

    def __reduce_allergen(self, allergen):
        '''
        For this allergen, find all recipes that contain that allergen. Then check the ingredients they share.
        If the shared ingredients is a single ingredient, then that ingredient contains that allergen.
        '''
        recipes_containing_allergen = self.recipes_by_allergen[allergen]

        shared_ingredients = self.unique_ingredients

        # remove the ingredients we've already identified
        already_identified_ingredients = self.identified_ingredients.keys()
        shared_ingredients = shared_ingredients.difference(
            already_identified_ingredients)

        for recipe in recipes_containing_allergen:
            ingredients_in_recipe = self.ingredients_by_recipe[recipe]
            shared_ingredients = shared_ingredients.intersection(
                ingredients_in_recipe)

        if len(shared_ingredients) == 1:
            # yay!
            ingredient = next(iter(shared_ingredients))
            self.identified_allergens[allergen] = ingredient
            self.identified_ingredients[ingredient] = allergen
            return True
        return False
    ...
    def reduce(self):
        while True:
            found = False
            for allergen in self.recipes_by_allergen.keys():
                if self.__reduce_allergen(allergen):
                    found = True
            if not found:
                break
        return

    def get_unidentified_allergens(self):
        return self.unique_ingredients.difference(self.identified_ingredients.keys())


def parse_recipes(puzzle_input):
    lines = puzzle_input.strip().split("\n")
    recipes = []
    for line in lines:
        recipes.append(Recipe.parse(line))
    return recipes


def solve(puzzle_input):
    recipes = parse_recipes(puzzle_input)
    recipe_reducer = RecipeReducer(recipes)
    recipe_reducer.reduce()
    unidentified_ingredients = recipe_reducer.get_unidentified_allergens()

    # part 1
    total = 0
    for recipe in recipes:
        matched_ingredients = recipe.ingredients.intersection(
            unidentified_ingredients)
        total += len(matched_ingredients)
    print(total)

    # part 2
    identified_allergens = list(recipe_reducer.identified_allergens.items())
    identified_allergens.sort(key=lambda x: x[0])
    print(','.join([x[1] for x in identified_allergens]))
    return

...
def round(grid, part2=False):
    ...
    def get_adjacent(x, y, grid):
        ...
        return [
            ...
            grid[y - 1][x + 0] if y > 0 else 0,
            grid[y - 1][x + 1] if y > 0 and x < max_x else 0,
            grid[y + 0][x - 1] if x > 0 else 0,
            grid[y + 0][x + 1] if x < max_x else 0,
            grid[y + 1][x - 1] if y < max_y and x > 0 else 0,
            grid[y + 1][x + 0] if y < max_y else 0,
            grid[y + 1][x + 1] if y < max_y and x < max_x else 0
        ]
    ...
    OCCUPIED_THRESHOLD = 5 if part2 else 4

    seat_changes = 0
    for y in range(0, len(grid)):
        for x in range(0, len(grid[y])):
            adjacent_spaces = get_visible(
                x, y, grid) if part2 else get_adjacent(x, y, grid)
            if grid[y][x] == EMPTY and adjacent_spaces.count(OCCUPIED) == 0:
                new_grid[y][x] = OCCUPIED
                seat_changes += 1
            elif grid[y][x] == OCCUPIED and adjacent_spaces.count(OCCUPIED) >= OCCUPIED_THRESHOLD:
                new_grid[y][x] = EMPTY
                seat_changes += 1
    return (new_grid, seat_changes)


def figure_out_final_seat_state(grid, part2=False):
    while True:
        result = round(grid, part2)
        grid = result[0]
        if not result[1]:
            break
    return grid
...
def count_occupied(grid):
    ...
    for y in range(0, len(grid)):
        for x in range(0, len(grid[y])):
            total_occupied += 1 if grid[y][x] == OCCUPIED else 0
    return total_occupied


def solve(puzzle_input):
    grid = figure_out_final_seat_state(
        [[y for y in x] for x in puzzle_input.strip().split("\n")])
